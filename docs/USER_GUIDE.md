...
Aide 是一个 AI Agent CLI 工具，支持多种大语言模型后端，可在终端中提供智能代码助手功能。具备文件读写、代码搜索、Shell 执行、任务追踪、代码智能、MCP 扩展等工具调用能力，内置 TUI 交互界面。
...
直接运行 `aide` (无参数)，如果配置文件不存在，会自动启动交互式设置向导，引导你完成:
...
# 指定 provider 和 model
aide -p anthropic -m claude-sonnet-4-6

# 带初始 prompt
aide chat "帮我重构 src/main.rs 中的错误处理"
```
...
```
┌──────────────────────────────────────────────┐
│  Aide | anthropic | claude-sonnet-4-6      │  状态栏
├──────────────────────────────────────────────┤
│  用户消息...                                   │  消息区
│  代理回复...                                   │
...
Aide 内置 13 个工具，Agent 会自动判断何时使用:
...
- 编辑 `~/.config/aide/settings.json` 手动配置
...
Aide 支持 Model Context Protocol (MCP)，可连接外部工具服务器扩展能力。

### 8.1 配置 MCP 服务器

在 `~/.config/aide/mcp_servers.json` 中添加:
...
# 加载指定会话
aide history <session-id>
```

会话文件位置:
| 平台 | 路径 |
|------|------|
| Linux | `~/.local/share/aide/sessions/` |
| macOS | `~/Library/Application Support/aide/sessions/` |
| Windows | `%APPDATA%/aide/sessions/` |

## 11. 常见问题

### Q: 提示 "Provider not configured"
A: 运行 `aide config init` 初始化配置，然后编辑配置文件填入 API key。
...
```bash
# 删除二进制
rm /usr/local/bin/aide       # Linux/macOS

# 删除配置和数据
rm -rf ~/.config/aide        # Linux
rm -rf ~/Library/Application\ Support/aide  # macOS
rmdir /s %APPDATA%\aide      # Windows
```
